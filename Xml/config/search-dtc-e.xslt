...
<xsl:variable name="dtclist">
	<xsl:for-each select="//servinfo">
		<xsl:if test="@dtccode!=''">
			<xsl:value-of select="@dtccode"/>
			<xsl:if test="following::servinfo[@dtccode!='']">
				<xsl:text>::</xsl:text>
			</xsl:if>
		</xsl:if>
	</xsl:for-each>
...
<xsl:variable name="sietitlelist_dtc">
	<xsl:for-each select="//servinfo">
		<xsl:if test="@dtccode!=''">
...
<xsl:variable name="sieidlist_dtc">
	<xsl:for-each select="//servinfo">
		<xsl:if test="@dtccode!=''">
...
var strDtcList = "<xsl:value-of select="$dtclist"/>"
	<xsl:value-of select="$newline"/>
var strSieIdList_Dtc = "<xsl:value-of select="$sieidlist_dtc"/>"
	<xsl:value-of select="$newline"/>
var strSieTitleList_Dtc = "<xsl:value-of select="$sietitlelist_dtc"/>"
	<xsl:value-of select="$newline"/>
var arrDtcs = strDtcList.split("::")
	<xsl:value-of select="$newline"/>
var arrSieList_Dtc = strSieIdList_Dtc.split(",")
	<xsl:value-of select="$newline"/>
var arrSieTitles_Dtc = strSieTitleList_Dtc.split("::")
	<xsl:value-of select="$newline"/>
...
<table border="0" width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<xsl:call-template name="getDtcSearchTitle">
			<xsl:with-param name="lang" select="$lang"/>
		</xsl:call-template>
		</div></td>
	</tr>
...
<tr>
<td align="left">
	<form class="searchconsole" name="searchdtc" onSubmit="runSearch2(); return false;">
	<table border="0" cellpadding="2" cellspacing="0">
	<tr valign="middle"><td align="left" colspan="2" style="padding-left:10px;">
		<xsl:call-template name="getUI_strSearchDtc">
			<xsl:with-param name="lang" select="$lang"/>
		</xsl:call-template>
	</td></tr>
